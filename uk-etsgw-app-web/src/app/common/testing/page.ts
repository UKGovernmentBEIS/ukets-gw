...
export class BasePage<C> {
  ...
  query<T extends keyof HTMLElementTagNameMap>(
    selector: T
  ...
  queryAll<T extends keyof HTMLElementTagNameMap>(
    selector: T
  ): HTMLElementTagNameMap[T][];
  ...
  getInputValue<T = string>(
    selector: string | HTMLInputElement | HTMLSelectElement
  ): T {
    return getInputValue(
      this.fixture,
      selector instanceof Element ? selector : this.sanitizeSelector(selector)
    );
  }
