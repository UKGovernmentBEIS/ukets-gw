 ...
 */
export function createSelector<S = any, SL = any>(
  projector: (state: S) => SL
): StateSelector<S, SL> {
 ...
 */
export function createAggregateSelector<
  S = any,
  IM1 = any,
  IM2 = any,
  SL = any
...
export function createAggregateSelector<S, SL = any>(
  ...args
): StateSelector<S, SL> {
...
export abstract class SignalStore<T> {
  ...
  updateState(dto: Partial<T>): void {
    this.setState({ ...this.state, ...dto });
  }
  ...
  rxSelect<SL = T[keyof T]>(selector: StateSelector<T, SL>): Observable<SL> {
    return runInInjectionContext(this.injector, () =>
      toObservable(this.select(selector))
    );
  }
